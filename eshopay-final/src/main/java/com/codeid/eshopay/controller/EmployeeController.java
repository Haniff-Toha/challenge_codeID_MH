...
public class EmployeeController extends BaseMultipartController<EmployeeDto, Integer>{
    ...
    public ResponseEntity<?> getDependents(@PathVariable Integer id){
        var dependentsDto = this.dependentService.findDependentByEmployeeId(id);
    ...
    @PostMapping("{id}/dependent/bulk")
    public ResponseEntity<?> createBulkDependent(@PathVariable Integer id,@RequestBody @Valid DependentBulkDto dependentBulkDto){
        var result = this.dependentService.bulkInsert(dependentBulkDto);

         var response = ApiResponse.builder()
                        .status(EnumStatus.Succeed.toString())
                        .message("createBulkDependent()")
                        .data(result)
                        .build();
